...
json.usage users: {
  total:          User.count,
  activeMonth:    User.where(created_at: ((Time.current - 30.days)...Time.current)).count,
  activeHalfyear: User.where(created_at: ((Time.current - 180.days)...Time.current)).count,
}
json.metadata({})
